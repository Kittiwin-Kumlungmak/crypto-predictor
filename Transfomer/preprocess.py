...
def data_split(source, range = [0.6, 0.2, 0.2]):
    ...
    train_df.to_csv('Data/train-btc-raw.csv', index = False)
    valid_df.to_csv('Data/valid-btc-raw.csv', index = False)
    test_df.to_csv('Data/test-btc-raw.csv', index = False)
...
def process_data(source, is_train = False):
    ...
    df = df[['Close', 'Volume', 'sin_hour', 'cos_hour', 'sin_day', 'cos_day', 'sin_month', 'cos_month']]
    ...
    if is_train:
        ...
        #save the scaler
        dump(scaler, 'train-scaler.joblib')

    if not is_train:
        try:
            scaler = load('train-scaler.joblib')
            df[['Close', 'Volume']] = scaler.transform(df[['Close', 'Volume']])
        except:
...
train_dataset = process_data('Data/train-btc-raw.csv', is_train= True)
valid_dataset = process_data('Data/valid-btc-raw.csv')
test_dataset = process_data('Data/test-btc-raw.csv')

train_dataset.to_csv('Data/train-btc-dataset.csv', index=False)
valid_dataset.to_csv('Data/valid-btc-dataset.csv', index=False)
test_dataset.to_csv('Data/test-btc-dataset.csv', index=False)